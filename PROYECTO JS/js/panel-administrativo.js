// --- ESTADO GLOBAL (Simulación de Base de Datos) ---
let medicosDB = [
...
let editandoMedicoIndex = null; // Para saber si creamos o editamos
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inicializarAccionesSecundarias();
});
    ...
    if(ctx) {
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Ingresos (S/)',
                    data: [1200, 1900, 1500, 2200, 1800, 900],
...
// --- 4. RENDERIZADO DE DATOS (CRUD y Tablas) ---
function cargarDatosSistema() {
    // Actualizar Estadísticas Dashboard
    document.getElementById('stat-pacientes').innerText = pacientesDB.length + 1450; // Fake number para volumen
    ...
    medicosDB.forEach((m, index) => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>
                    <button class="btn-accion btn-ver" onclick="verPerfil('${m.nombre}')">Ver</button>
                    <button class="btn-accion btn-editar" onclick="editarMedico(${index})">Editar</button>
                    <button class="btn-accion btn-eliminar" onclick="eliminarMedico(${index})">Quitar</button>
                </td>
            </tr>
        `;
    });
    ...
    pacientesDB.forEach(p => {
        tbody.innerHTML += `
            <tr>
                ...
                <td>${p.nombre}</td>
                <td>${p.ultima}</td>
                <td><span class="badge ${p.historial === 'Normal' ? 'badge-success' : 'badge-espera'}">${p.historial}</span></td>
                <td>
                    <button class="btn-accion btn-ver" onclick="verPerfil('${p.nombre}')">Historial Clínico</button>
                </td>
            </tr>
        `;
    });
...
function renderTablaCitas() {
    ...
    citasDB.forEach((c, index) => {
        ...
        let btnCancelar = c.estado !== 'Cancelada' 
            ? `<button class="btn-accion btn-cancelar" onclick="cancelarCita(${index})">Cancelar</button>` 
            : `<span style="font-size:0.8em; color:gray;">Sin acciones</span>`;

        // Para Dashboard (resumido)
        if(tbodyDash) {
            tbodyDash.innerHTML += `
                <tr>
                    <td>${c.paciente}</td>
        ...
        // Para Vista Citas Activas (Completo)
        htmlContent += `
            <tr>
                <td>${c.id}</td>
                <td>${c.paciente}</td>
...
// --- 5. LÓGICA DE CRUD MÉDICOS Y MODALES ---
function inicializarModalesYFormularios() {
    ...
    if(btnNuevoMedico) {
        btnNuevoMedico.addEventListener('click', () => {
            ...
            formMedico.reset();
            document.getElementById('titulo-modal-medico').innerText = "Registrar Nuevo Médico";
            modalMedico.hidden = false;
    ...
    btnCerrar.forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            modalMedico.hidden = true;
            modalMedico.style.display = 'none';
        });
    });

    // Submit Médico (Crear o Editar)
    if(formMedico) {
        formMedico.addEventListener('submit', (e) => {
            ...
            const estado = document.getElementById('medico-estado').value;

            if (editandoMedicoIndex !== null) {
                // Editar existente
                medicosDB[editandoMedicoIndex].nombre = nombre;
                medicosDB[editandoMedicoIndex].especialidad = especialidad;
                medicosDB[editandoMedicoIndex].estado = estado;
                Swal.fire('Actualizado', 'Datos del médico actualizados.', 'success');
            } else {
                // Crear nuevo
                const nuevoId = "MED-00" + (medicosDB.length + 1);
                medicosDB.push({ id: nuevoId, nombre, especialidad, estado });
                Swal.fire('Registrado', 'El médico ha sido agregado al sistema.', 'success');
            }

            modalMedico.hidden = true;
            modalMedico.style.display = 'none';
            cargarDatosSistema(); // Recarga las tablas
        });
    }

    // Configuración general form
    document.getElementById('form-configuracion')?.addEventListener('submit', (e) => {
        e.preventDefault();
        Swal.fire('Guardado', 'Los ajustes se han guardado exitosamente.', 'success');
    });
}

// Funciones globales invocadas desde los botones onclick en el HTML inyectado
window.editarMedico = function(index) {
    editandoMedicoIndex = index;
    const m = medicosDB[index];
    ...
    document.getElementById('medico-especialidad').value = m.especialidad;
    document.getElementById('medico-estado').value = m.estado;
...
window.eliminarMedico = function(index) {
    Swal.fire({
        title: '¿Estás seguro?',
        text: `Vas a dar de baja a ${medicosDB[index].nombre}`,
        icon: 'warning',
        showCancelButton: true,
...
window.cancelarCita = function(index) {
    Swal.fire({
        title: 'Cancelar Cita',
        text: `¿Seguro que deseas cancelar la cita de ${citasDB[index].paciente}?`,
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            citasDB[index].estado = 'Cancelada';
            cargarDatosSistema();
            Swal.fire('Cancelada', 'La cita fue cancelada correctamente.', 'info');
        }
    });
};
...
window.verPerfil = function(nombre) {
    Swal.fire({
        ...
        text: `Estás viendo la ficha de: ${nombre}\n(Esta acción redirigiría a su expediente completo en un sistema real)`,
        icon: 'info'
    });
};


// --- 6. BUSCADORES EN TIEMPO REAL ---
    ...
    const buscarEnTabla = (inputId, tbodyId) => {
        ...
        input.addEventListener('keyup', function() {
            ...
            filas.forEach(fila => {
                ...
                fila.style.display = texto.includes(filas) ? '' : 'none'; // Aquí había un bug en tu código, "filas" -> "filtro"
                fila.style.display = texto.includes(filtro) ? '' : 'none'; 
            });
        });
    }

    buscarEnTabla('buscar-medico', 'tbody-medicos');
    buscarEnTabla('buscar-paciente', 'tbody-pacientes');
}

// --- 7. DESCARGA DE REPORTES Y EXTRAS ---
function inicializarAccionesSecundarias() {
    const descargarReporteSimulado = (e) => {
        ...
        Swal.fire({
            ...
            // Simular creación de un archivo y descarga
            ...
            a.setAttribute('download', 'Reporte_CURAR_2026.csv');
    ...
    document.getElementById('btn-backup')?.addEventListener('click', () => {
        Swal.fire('Backup Iniciado', 'La base de datos se está respaldando en el servidor.', 'success');
    });
}