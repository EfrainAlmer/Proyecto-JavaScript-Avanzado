...
class CitaMedico {
    constructor({ id, paciente, especialidad, fecha, hora, estado, medico, sede }) {
        this.id = id;
        this.paciente = String(paciente).trim();
        this.especialidad = especialidad || "General";
        this.ubicacion = sede || "Sede Ate"; 
        this.fecha = fecha;
        this.hora = hora;
        this.estado = estado;
        this.medico = medico;
...
function inicializarPanelMedico() {
    try {
        ...
        const navDocName = document.getElementById("pm-doc-name");
        const navProfile = document.getElementById("pm-profile-name");
        if (navDocName) navDocName.textContent = `Buenos días, ${sesion.nombre}`;
        if (navProfile) navProfile.textContent = sesion.nombre;
        ...
        calcularMetricas();
        renderizarCitas(misCitas);
        renderizarProximosPacientes(misCitas);

    } catch (error) {
        console.error("Error cargando panel médico:", error.message);
    }
}
...
function calcularMetricas() {
    const metricas = misCitas.reduce((acc, cita) => {
        ...
        if (estado.includes("atendid") || estado.includes("completad")) {
            acc.pacientesUnicos++;
        }
        return acc;
    }, { citasTotales: 0, pacientesUnicos: 0 });

    const elApp = document.getElementById("metric-appointments");
    const elPat = document.getElementById("metric-patients");
    if (elApp) elApp.textContent = Math.max(0, metricas.citasTotales);
    if (elPat) elPat.textContent = Math.max(0, metricas.pacientesUnicos + 578);
}
...
function renderizarCitas(citas) {
    ...
    if (citas.length === 0) {
        contenedor.innerHTML = `<p style="text-align: center; color: #888;">No hay citas programadas para hoy.</p>`;
        return;
    }
    ...
    citas.forEach(cita => {
        ...
        fila.innerHTML = `
            <div><strong>👤 ${cita.paciente}</strong></div>
            <div>${cita.ubicacion}</div>
            <div>${cita.fecha}</div>
            <div>${cita.hora}</div>
            <div style="display: flex; gap: 10px; align-items: center;">
                ${esAtendida 
                    ...
                    : `<button class="btn-atender" data-id="${cita.id}" style="background: #10b981; color: white; border: none; padding: 6px 14px; border-radius: 6px; cursor: pointer; font-weight: bold;">Atender</button>`
                }
            </div>
        `;
    ...
    document.querySelectorAll(".btn-atender").forEach(btn => {
        btn.addEventListener("click", (e) => procesarAtencion(e.target.dataset.id));
    });
}
...
function renderizarProximosPacientes(citas) {
    ...
    citas.slice(0, 4).forEach((cita, index) => {
        ...
        contenedor.innerHTML += `
            <div class="${clase}" style="cursor: pointer;">
                👤<br>
                <strong>${cita.paciente.split(" ")[0]}</strong><br>
                <small>${cita.hora.split(" ")[0]}</small>
            </div>
        `;
    });
}
...
function configurarEventos() {
    ...
    if (inputBuscar) {
        inputBuscar.addEventListener("input", (e) => {
            ...
            try {
                const patron = new RegExp(termino, "iu");
                const filtradas = misCitas.filter(cita => patron.test(cita.paciente) || patron.test(cita.ubicacion));
...
function configurarBotonesInterfaz() {
    ...
    const diasCalendario = document.querySelectorAll(".pm-day");
    diasCalendario.forEach(dia => {
        dia.addEventListener("click", (e) => {
            diasCalendario.forEach(d => d.classList.remove("pm-day--active"));
            e.currentTarget.classList.add("pm-day--active");
        });
    });
}
// CÁLCULO DINÁMICO DE LA SEMANA ACTUAL PARA EL CALENDARIO
document.addEventListener("DOMContentLoaded", () => {
    actualizarCalendarioDinamico();
});
...
function actualizarCalendarioDinamico() {
    ...
    // Tomamos la fecha actual (o simulada en este entorno)
    const hoy = new Date(); 
    ...
    for (let i = 0; i < 5; i++) {
        ...
        htmlCalendario += `
            <div class="${claseActiva}" data-fecha="${d.toISOString().split('T')[0]}">
                ${nombresDias[i]}<br><strong>${numeroDia}</strong>
            </div>
        `;
    }
    ...
    // Añadir interactividad para hacer clic en los días del calendario
    document.querySelectorAll(".pm-day").forEach(dia => {
        dia.addEventListener("click", (e) => {
            document.querySelectorAll(".pm-day").forEach(d => d.classList.remove("pm-day--active"));
            e.currentTarget.classList.add("pm-day--active");
            
            // Opcional: Aquí puedes filtrar las citas de la tabla según la fecha seleccionada en el futuro
            const fechaSeleccionada = e.currentTarget.dataset.fecha;
            console.log("Filtrando para la fecha:", fechaSeleccionada);
        });
    });
}